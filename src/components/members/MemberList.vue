...
export default {
  ...
  methods: {
    filterList: function(res) {
      window.Event.$emit('any:listen-for-enter:add');
    ...
    enterListener: function(event) {
      ...
      if (event.code === 'Enter') {
        ...
        window.Event.$emit('any:listen-for-enter:remove');
      }
    },
  ...
  watch: {
    memberIds: function(newValue, oldValue) {
      if (this.stagedMemberIds.length === 0) this.members = this.filteredMembers = this.$parent.members.filter(m => this.memberIds.includes(m.id));
    },

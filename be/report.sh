...
# -- fetch all companionships where there is only a single minister assigned
read -r -d '' sql <<-'EOF'
select district_id, companionship_id, count(companionship_id) as cnt from district_assignments where type = 'minister' group by district_id, companionship_id having count(companionship_id) < 2
EOF

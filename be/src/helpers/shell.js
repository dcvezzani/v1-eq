import { exec } from 'child_process';
import fs from 'fs';
import moment from 'moment';
import { sendShellCommand as sendShellCommandAction } from './../actions/shell'

const V1_DIR_TEXT_CACHE = "/Users/davidvezzani/projects/v1-eq/be/data/v1-dir-text-cache.txt";

// curl http://localhost:3000/sms/text -XPOST --data '{"title":"Bro.", "lastname":"Johansen", "phone":"8013333333", "visitTime":"3:30pm", "visitDate":"7/29"}' -H "Content-Type: application/json"
// curl http://localhost:3000/sms/text -XPOST --data '{"data":""}' -H "Content-Type: application/json"

// todo: this is really "send text message"
export const sendShellCommand = (req, res, next) => {
  sendShellCommandAction(req.body.data, V1_DIR_TEXT_CACHE, (err, { cmd, stdout }) => {
    res.json({ err, cmd, stdout });
  });
};

